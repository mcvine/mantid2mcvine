...
intercepted by CHESS detector system.
...
class Neutrons2Events:
    ...
    def __init__(
        ...
        self.instrument_xml = instrument_xml
        self.tofbinsize = tofbinsize
        return

    # main methods
    def run(self, neutrons, workdir, nodes, ncount=None):
        neutrons = os.path.abspath(neutrons)
        workdir = os.path.abspath(workdir)
        if os.path.exists(workdir):
            raise IOError("%s already exists" % workdir)
        os.makedirs(workdir)
        eventdat = self.sendneutronstodetsys(
    ...
    def sendneutronstodetsys(
        ...
        args = {
            ...
            'output-dir': 'out',
            'detsys.tofparams': '0,1.,%s' % (1e-6*self.tofbinsize,), 
            'detsys.instrumentxml': self.instrument_xml,
